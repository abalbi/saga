package Libido::Persona::Fabrica;
use base qw(Persona::Fabrica);
use strict;
use Data::Dumper;
use DateTime;
...
sub _hacer {
  ...
  delete $params->{nombre} if($params->{nombre} =~ /^!/);
  foreach my $key (sort keys %{$persona_params}) {
    $params->{$key} = $persona_params->{$key} if not exists $params->{$key};
  }
}
...
sub dar_acentos {
  ...
  $persona_params->{acentos} = Saga::despachar('Persona::Valor')->new({key => 'acentos', valor => $acentos});
}
...
sub dar_rasgos {
  ...
  $persona_params->{rasgos} = Saga::despachar('Persona::Valor::Rasgos')->new({key => 'rasgos', valor => $rasgos});
}
...
sub dar_animo {
  ...
  $valor = 6 if $numero >= 4;
  $valor = 7 if $numero >= 7;
  $persona_params->{animo}= Saga::despachar('Persona::Valor::Animo')->new({key => 'animo', valor => $valor});
}
...
sub dar_salud {
  ...
  $persona_params->{salud}= Saga::despachar('Persona::Valor::Salud')->new({key => 'salud', valor => $valor});
}
...
sub dar_aptitudes {
  ...
  $persona_params->{cuerpo} = Saga::despachar('Persona::Valor')->new({key => 'cuerpo', valor => $valores->[0]});
  $persona_params->{mente} = Saga::despachar('Persona::Valor')->new({key => 'mente', valor => $valores->[1]});
  $persona_params->{espiritu} = Saga::despachar('Persona::Valor')->new({key => 'espiritu', valor => $valores->[2]});
  ...
  $persona_params->{sentidos} = Saga::despachar('Persona::Valor')->new({key => 'sentidos', valor => $sentidos});
  $persona_params->{sociabilidad} = Saga::despachar('Persona::Valor')->new({key => 'sociabilidad', valor => $sociabilidad});
}
...
sub dar_ascendente_erotica {
  ...
  $persona_params->{ascendente_erotica} = Saga::despachar('Persona::Valor')->new({key => 'ascendente_erotica', valor => $ascendente_erotica});
  my $elecciones = [
    Saga::azar([qw(h m)]),
    Saga::azar([qw(h m)]),
    Saga::azar([qw(h m)]),
  ...
  $persona_params->{elecciones_erotica} = Saga::despachar('Persona::Valor')->new({key => 'elecciones_erotica', valor => $elecciones});
}
...
sub dar_nacimiento {
  ...
  $persona_params->{nacimiento} = Saga::despachar('Persona::Valor')->new({key => 'nacimiento', valor => $dt->datetime});
}

sub dar_sexo {
  my $self = shift;
  my $persona_params = shift;
  $persona_params->{sexo} = Saga::despachar('Persona::Valor')->new({key => 'sexo', valor => Saga::azar([qw(f m)])});
...
sub dar_nombre {
  my $self = shift;
  my $persona_params = shift;
  return if $persona_params->{nombre} && $persona_params->{nombre} !~ /^\!/;
  my $nombres = $persona_params->{sexo}->valor eq 'f' ? $nombres_mujer : $nombres_hombre;
  my $nombre = Saga::azar($nombres);
  $persona_params->{nombre} = Saga::despachar('Persona::Valor')->new({key => 'nombre', valor => $nombre});
